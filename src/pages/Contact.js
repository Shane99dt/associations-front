...
const Contact = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const request = await fetch(`http://localhost:5000/associations/contact`, {
      method: 'POST',
    ...
    if(request.status === 201){
      navigate(`/associations/association/${slug}`)
    }else{
      alert('Bad request')
    }
  }
      ...
      <form className="w-full max-w-lg m-auto py-5" onSubmit={handleSubmit}>
        {/* <div className="flex flex-wrap -mx-3 mb-6"> */}
          ...
          <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
            <label className="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" htmlFor="grid-association">
              Association
            </label>
            <div className="relative">
              <select className="block appearance-none w-full bg-gray-200 border dark:border-gray-600 dark:bg-gray-700 border-gray-200 py-3 px-4 pr-8 rounded leading-tight dark:text-gray-400 focus:outline-none focus:bg-white focus:border-gray-500" id="grid-association" onChange={handleChangeSelect} value={slug}>
